...
class Test(InferenceTest):
    ...
    def get_train_data(n_samples=2000, n_features=20, fit_intercept=True):
        np.random.seed(123)
        weights0 = weights_sparse_gauss(n_features, nnz=5) / 3.
        if fit_intercept:
            intercept0 = -0.1
        else:
            intercept0 = None
    ...
    def test_PoissonRegression_fit(self):
        ...
        for i, (solver, penalty, fit_intercept) \
                in enumerate(product(PoissonRegression._solvers.keys(),
                                     PoissonRegression._penalties.keys(),
                                     fit_intercepts)):
            if fit_intercept:
                y_ = y + intercept0
            else:
    ...
    def test_PoissonRegression_warm_start(self):
        """...Test PoissonRegression warm start
        """
        X, y, weights0, intercept0 = Test.get_train_data()
        fit_intercepts = [True, False]
        cases = itertools.product(PoissonRegression._solvers.keys(),
                                  fit_intercepts)
    ...
    def test_safe_array_cast(self):
        ...
        np.testing.assert_array_equal(self.X, PoissonRegression._safe_array(self.X))

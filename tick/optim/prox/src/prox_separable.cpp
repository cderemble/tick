...
void ProxSeparable::call(const ArrayDouble &coeffs,
  ...
  for (ulong i = 0; i < sub_coeffs.size(); ++i) {
    ...
    call(i, sub_coeffs, step, sub_out);
  }
}
...
void ProxSeparable::call(const ArrayDouble &coeffs,
  ...
  ArrayDouble sub_out = view(out, start, end);
  for (ulong i = 0; i < sub_coeffs.size(); ++i) {
      call(i, sub_coeffs, step[i], sub_out);
  }
}

double ProxSeparable::call(double x,
                           double step) const {
...
void ProxSeparable::call(ulong i,
                         const ArrayDouble &coeffs,
                         double step,
                         ArrayDouble &out) const {
  out[i] = call(coeffs[i], step);
}
...
void ProxSeparable::call(ulong i,
                         ...
                         ArrayDouble &out,
                         ulong n_times) const {
  out[i] = call(coeffs[i], step, n_times);
}
...
double ProxSeparable::value(const ArrayDouble &coeffs,
  ...
  ArrayDouble sub_coeffs = view(coeffs, start, end);
  for (ulong i = 0; i < sub_coeffs.size(); ++i) {
    val += value(i, sub_coeffs);
  }
  return strength * val;
}

double ProxSeparable::value(double x) const {
  TICK_CLASS_DOES_NOT_IMPLEMENT(get_class_name());
}

double ProxSeparable::value(ulong i,

...
#ifndef TICK_MODEL_SCCS_H
#define TICK_MODEL_SCCS_H
...
class ModelSCCS : public ModelLipschitz  {
 ...
 protected:
  ...
  ulong n_features;
//  const bool fit_intercept = false;  // No intercept for now.
 ...
 public:
  ...
  static inline long double sumExpMinusMax(ArrayDouble &x, double x_max) {
    double sum = 0;
    for (ulong i = 0; i < x.size(); ++i) sum += exp(x[i] - x_max);
    return sum;
  }

  static inline long double logSumExp(ArrayDouble &x) {
    double x_max = x.max();
  ...
  static inline void softMax(ArrayDouble &x, ArrayDouble &out) {
    double x_max = x.max();
    long double sum = sumExpMinusMax(x, x_max);
